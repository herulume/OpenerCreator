...
namespace OpenerCreator.Helpers
{
    ...
    public class Actions : IActionManager
    {
        ...
        public List<uint> NonRepeatedIdList() => actionsSheet.Select(a => a.RowId).Where(id => id != 0).ToList();

        public string GetActionName(uint id) => actionsSheetDictionary[id].Name.ToString();

        public LuminaAction GetAction(uint id) => actionsSheetDictionary[id];

        public ushort GetActionIcon(uint id) => actionsSheetDictionary[id].Icon;

        public List<uint> GetNonRepeatedActionsByName(string name, Jobs job) => actionsSheet
            .AsParallel()
            .Where(a =>
        ...
        public bool SameActionsByName(string name, uint aId) => actionsSheetDictionary[aId].Name.ToString().ToLower().Contains(name.ToLower());
